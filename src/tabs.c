...
void setup_credits_tab(struct credits_tab *credits)
{
	...
	credits->is_not_after = nk_false;
	credits->is_not_before = nk_false;
...
void draw_decoders_tab(struct nk_context *ctx, int *tab_screen_height, struct decoders_tab *decoders)
{
	*tab_screen_height = 472;
	...
	if(nk_group_begin(ctx, "608 Decoder", NK_WINDOW_TITLE|NK_WINDOW_BORDER|NK_WINDOW_NO_SCROLLBAR))
	{
		...
		{
			static int option = CHANNEL_1;
			nk_layout_row_dynamic(ctx, 20, 1);
			if(nk_option_label(ctx, "Extract captions from channel 1", option == CHANNEL_1))
			{
				option = CHANNEL_1;
				decoders->channel = CHANNEL_1;
			}
			nk_layout_row_dynamic(ctx, 20, 1);
			if(nk_option_label(ctx, "Extract captions from channel 2", option == CHANNEL_2))
			{
				option = CHANNEL_2;
	...
	{
		static int option = TELETEXT;
		nk_layout_row_dynamic(ctx,20, 2);
		if(nk_option_label(ctx, "Teletext", option == TELETEXT)){
			option = TELETEXT;
			decoders->teletext_dvb = TELETEXT;
		}
		if(nk_option_label(ctx, "DVB", option == DVB)){
			option = DVB;
